import * as S from "../../styles";

const Catequese = () => {
  return (
    <S.CatequeseContainer>
      <div>
        <a href="/trindade" target="_blank">
          <S.CatequeseImage
            src="assets/trindade/trindade.jpg"
            alt="Santíssima Trindade"
          />
        </a>
        <S.BigText>
          <a href="/trindade" target="_blank">
            Santíssima Trindade
          </a>
        </S.BigText>
      </div>
      <div>
        <a href="/dogmascatolicos" target="_blank">
          <S.CatequeseImage
            src="assets/dogmas_catolicos/dogmas_catolicos.jpg"
            alt="Dogmas Católicos"
          />
        </a>
        <S.BigText>
          <a href="/dogmascatolicos" target="_blank">
            Dogmas da Igreja Católica
          </a>
        </S.BigText>
      </div>
      <div>
        <a href="">
          <S.CatequeseImage
            src="assets/construcao.png"
            alt="Os 10 Mandamentos"
          />
        </a>
        <S.BigText>
          <a href="">Os 10 Mandamentos</a>
        </S.BigText>
      </div>
      <div>
        <a href="/sacramentos" target="_blank">
          <S.CatequeseImage
            src="assets/rosario/eucaristia_anjos.jpg"
            alt="Os 7 Sacramentos"
          />
        </a>
        <S.BigText>
          <a href="/sacramentos" target="_blank">
            Os 7 Sacramentos
          </a>
        </S.BigText>
      </div>
      <div>
        <a href="">
          <S.CatequeseImage
            src="assets/construcao.png"
            alt="Os dons do Espírito Santo"
          />
        </a>
        <S.BigText>
          <a href="">Os Dons do Espírito Santo</a>
        </S.BigText>
      </div>
      <div>
        <a href="/liturgia" target="_blank">
          <S.CatequeseImage src="assets/liturgia/missa.jpg" alt="Liturgia" />
        </a>
        <S.BigText>
          <a href="/liturgia" target="_blank">
            Liturgia
          </a>
        </S.BigText>
      </div>
      <div>
        <a href="">
          <S.CatequeseImage src="assets/construcao.png" alt="Virtudes" />
        </a>
        <S.BigText>
          <a href="">Virtudes</a>
        </S.BigText>
      </div>
      <div>
        <a href="/obrasmisericordia" target="_blank">
          <S.CatequeseImage
            src="assets/obras_misericordia/filantropia.jpg"
            alt="As Obras de Misericórdia"
          />
        </a>
        <S.BigText>
          <a href="/obrasmisericordia" target="_blank">
            As Obras de Misericórdia
          </a>
        </S.BigText>
      </div>
      <div>
        <a href="/oracoes" target="_blank">
          <S.CatequeseImage src="assets/oracoes/oracao.jpg" alt="Orações" />
        </a>
        <S.BigText>
          <a href="/oracoes" target="_blank">
            Orações
          </a>
        </S.BigText>
      </div>
      <div>
        <a href="">
          <S.CatequeseImage
            src="assets/construcao.png"
            alt="Os 4 Dogmas Marianos"
          />
        </a>
        <S.BigText>
          <a href="">Os 4 Dogmas Marianos</a>
        </S.BigText>
      </div>
      <div>
        <a href="">
          <S.CatequeseImage
            src="assets/construcao.png"
            alt="Graus de Adoração"
          />
        </a>
        <S.BigText>
          <a href="">Graus de Adoração</a>
        </S.BigText>
      </div>
      <div>
        <a href="">
          <S.CatequeseImage src="assets/construcao.png" alt="Pecados" />
        </a>
        <S.BigText>
          <a href="">Pecados</a>
        </S.BigText>
      </div>
      <div>
        <a href="">
          <S.CatequeseImage src="assets/construcao.png" alt="Pós Morte" />
        </a>
        <S.BigText>
          <a href="">Pós Morte</a>
        </S.BigText>
      </div>
    </S.CatequeseContainer>
  );
};

export default Catequese;
